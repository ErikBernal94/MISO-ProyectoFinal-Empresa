    ...
    crear(contrato){
        return new Promise(async (resolve,reject)=>{
            try {
                ...
                if(contratoExistente.length > 0) reject({codigoError: 419, mensaje:'El contrato ya existe para el empleado y la empresa ingresados'});
                let rolExistente = await metadataData.obtenerRol(contrato.idRol);
                if(!rolExistente || rolExistente.length === 0) reject({codigoError: 419, mensaje:'El rol no existe'});
                let proyectoExistente = await proyectoData.obtenerPorId(contrato.idProyecto)
                if(!proyectoExistente || proyectoExistente.length === 0) reject({codigoError: 419, mensaje:'El proyecto no existe'});
